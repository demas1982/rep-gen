      ...
      <div  class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
        <md-card style="z-index: 2;">
          <md-card-header data-background-color="green">
            ...
            <p class="category">Задайте отчет и нажмите кнопку "Добавить отчет". Рузельтат можно наблюдать в форме ниже</p>
          </md-card-header>
          <md-card-content sty>
            <div class="row">
              <md-card-content>
                <div class="form-group">
                  ...
                  <select data-live-search="true" @change="reloadTable(selectedReport)" id="selectedReport" class="form-control selectpicker show-tick" v-model="selectedReport" data-style="btn btn-link">
                    <option data-hidden="true"></option>
                    <option v-for="report of reports" :value="report.id">{{report.name}}</option> 
                  </select>
              </div>
...
export default {
  ...
  mounted(){
    ...
    .then(response => {
      ...
      this.$nextTick(function(){ $('#selectedReport').selectpicker('refresh'); });
    });
    var today = new Date();
